...
class PartialParse(object):
    ...
    def get_oracle(self, graph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        elif graph.nodes[self.stack[-1]]['head'] == self.stack[-2] and \
        (len(list(get_right_deps(graph.nodes[self.stack[-1]]))) == 0 or \
         self.next > max(get_right_deps(graph.nodes[self.stack[-1]]))):
